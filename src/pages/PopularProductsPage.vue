<template>
  <div class="row" style="text-align: center">
    <h4 class="col-12">POPULAR PRODUCTS</h4>
  </div>
  <div class="q-pa-md">
  ...
  <div v-else class="col text-center" style="height: 90vh; padding-top: 7%">
    ...
    <h2>There are no popular products</h2>
...
export default {
  ...
  async created() {
    apiClient.request('get', `/multiple-tables/all-popular-products`, null, null).then(
      result => {
        let tempResult = [];
  ...
  methods: {
    matchHeight() {
      if (this.filteredProducts.length < 6) {
        this.pageHeight.height = "90vh";
      } else {
        this.pageHeight.height = "auto";
      }
    },
    ...
    async search() {
      let url = '/multiple-tables/all-popular-products?productName=' + this.searchText;
      if (this.searchText !== null) {
        ...
        Notify.create({
          color: "negative",
          icon: "warning",
          message: "Enter a product name to be searched!"
        });
      }
    },
    returnOnHomePage() {
