...
export default {
  emits: ["clearFilters", "viewProduct"],
  props: {
    pSearchText: {
      type: String,
      required: false,
  ...
  async created() {
    ...
    let url = '/multiple-tables/all-products-by-product-name?productName=' + this.searchText;
      ...
      await apiClient.request('get', url, null, null).then(
        result => {
          ...
          for (let numberKey in tempResult) {
            tempResult[numberKey].tempId = tempResult[numberKey].id;
  ...
  methods: {
    ...
    viewProduct(product) {
      ...
      this.$emit("viewProduct", routerPushValues);
    },
    returnOnHomePage() {
      this.$router.push({
        path: "/",
