<template>
  <div class="q-pa-md row items-start q-gutter-md product-info-page">
    <q-card class="product-card-info" flat bordered>
      <div class="on-sale-sticker" v-if="caseFan.saleType !== null">{{ getSaleType() }}</div>
      <q-card-actions>
        <q-img v-if="caseFan.image !== null && caseFan.image !== ''"
               :src=caseFan.image
               width="50%"
               height="50%"
        />
        ...
        <div class="text-h4 q-mt-sm q-mb-xs product-info">
          <q-separator/>
          <p>Fan Size: {{ caseFan.fanSize }} mm</p>
          <q-separator/>
          <p>Color: {{ caseFan.color }}</p>
          <q-separator/>
          <p>Rpm: {{ caseFan.rpm }}</p>
          <q-separator/>
          <p>Noise level: {{ caseFan.noiseLevel }}</p>
          <q-separator/>
          <p>Manufacturer: {{ caseFan.manufacturer }}</p>
          <q-separator/>
          <p>Total Draw Power: {{ caseFan.tdp }}W</p>
          <q-separator/>
          <div v-if="checkIfRatingIsLoaded" class="q-pa-md rating">
            <q-rating
              v-model="caseFanRating"
      ...
      <q-separator/>
      <q-card-actions v-if="caseFan.description !== null">
        <div class="text-h6 q-mt-sm q-mb-xs">
          <p>Description:</p>
          {{ caseFan.description }}
        </div>
      </q-card-actions>
      <q-separator v-if="caseFan.description !== null"/>
      <q-card-actions>
        <div class="text-h4">
          Amount available: {{ caseFan.quantity }}
        </div>
        <q-space/>
        <div class="text-h4 q-mr-sm">
          Price:
        </div>
        <div v-if="caseFan.saleType !== null" class="text-h4 price-color">
          {{ this.priceWithDots(caseFan.price) + ' ' + caseFan.currency }}
        </div>
...
export default {
  ...
  async created() {
    await apiClient.request('get', '/case-fan/get-case-fan-average-rating?caseFanId=' + this.caseFan.id, null, null)
      .then(
        result => {
          this.caseFanRating = result;
        }
      );
    if (this.checkIfLoggedIn) {
      ...
      for (let tempPurchasedProduct of tempPurchasedProducts) {
        for (let tempPurchasedProductElement of tempPurchasedProduct) {
          if (tempPurchasedProductElement.productId === this.caseFan.id) {
  ...
  computed: {
    ...
    checkIfRatingIsLoaded() {
      return this.caseFanRating !== null;
    }
  },
  methods: {
    ...
    getSaleType() {
      let saleType = this.caseFan.saleType;
    ...
    async addRating() {
      if (!this.checkIfLoggedIn) {
        ...
        let httpMethod = 'post';
        if (this.caseFanRating !== null) {
          httpMethod = 'put';
        }
        let productRating = {
          productId: this.caseFan.id,
          productType: "case-fan",
          rating: this.caseFanRating,
          userId: this.securityStore.user.id
        }
        let tempProductRating = null;
        await apiClient.request('get', '/product-rating/find-by-user-id-and-product-id?userId=' + this.securityStore.user.id + '&productId=' + this.caseFan.id, null, null)
          .then(
            result => {
